...
# ── Display name mapping: raw BANK_NAME → executive report name ──
# UPDATE THIS when new banks/FIs are onboarded
NAME_MAP = {
    "Awash Bank":     "Awash",
    "Dashen Bank":    "Dashen",
    "COOP":           "CBO",
...
# FIs that always appear in the report even with no raw data
PERMANENT_FIS = [
...
def generate_qr_report(input_path: str, output_path: str, report_date: datetime):
    ...
    for _, r in df.iterrows():
        ...
        rows.append({
            "name": display_name,
            "is_new": raw_name not in NAME_MAP,
            "iss_cnt": iss_cnt, "iss_amt": iss_amt,
            "acq_cnt": acq_cnt, "acq_amt": acq_amt,
        })
        seen_names.add(display_name)

        # Auto-promote every FI seen in raw data into permanent list
    ...
    for fi in PERMANENT_FIS:
        if fi not in seen_names:
            rows.append({"name": fi, "is_new": False,
                         "iss_cnt": 0, "iss_amt": 0.0,
                         "acq_cnt": 0, "acq_amt": 0.0})

    # Sort A→Z
    rows.sort(key=lambda x: x["name"].lower())

    if unmapped:
        print("\n⚠️  UNMAPPED / NEW FI(s) DETECTED — please update NAME_MAP:")
        for u in unmapped:
            print(f"   • {u}")
        print("   These rows are highlighted in YELLOW in the report.\n")
    else:
        print("✅ All FIs matched — no new entries detected.")
    ...
    # Header rows (start at col E = col 5, matching sample)
    ...
    ws["F7"] = "No.Transactions"
    ws["G7"] = "Values"
    ws["H7"] = "No.Transactions"
    ws["I7"] = "Values"
    ...
    for i, row in enumerate(rows):
        r = data_start + i
        name_fill = make_fill(YELLOW) if row["is_new"] else make_fill(GREEN)

        ws[f"E{r}"] = row["name"]
        ws[f"E{r}"].fill = name_fill
    ...
    # Column widths & row heights (match sample)
    ...
    ws.row_dimensions[5].height = 14.45
    ws.row_dimensions[6].height = 14.45
...
if __name__ == "__main__":
    input_file  = sys.argv[1] if len(sys.argv) > 1 else "/mnt/user-data/uploads/QR_success_for_source_and_destination_1_.xlsx"
    ...
    generate_qr_report(input_file, output_file, report_date)
